...
import { NalType, NalUnit } from '../elementary/nal-unit';
import { H264Remuxer } from '../remuxer/h264-remuxer';
import { ExpGolomb } from '../util/exp-golomb';

const Log = getTagged('nalu:h264');

export class H264Parser {
  public remuxer: H264Remuxer;
  public firstFound: boolean;
  public track: { [key: string]: any };
  ...
  public msToScaled(timestamp: number): number {
    return (timestamp - this.remuxer.timeOffset) * this.remuxer.scaleFactor;
  }

  public parseSPS(sps: Uint8Array): void {
  ...
  public parsePPS(pps: ArrayBuffer): void {
    this.track.pps = [new Uint8Array(pps)];
  }

  public parseNAL(unit: NalUnit): boolean {
    if (!unit) return false;

    let push: boolean = null;
    // Log.log(unit.toString());
    switch (unit.type) {
      case NalType.NDR:
      case NalType.IDR:
        unit.sliceType = H264Parser.parceSliceHeader(unit.data.subarray(5));
        if (unit.isKeyframe && !this.firstFound) {
          this.firstFound = true;
        }
        push = this.firstFound ? true : false;
        break;
      case NalType.PPS:
        push = false;
        if (!this.track.pps) {
          this.parsePPS(unit.data.subarray(4));
      ...
      case NalType.SPS:
        push = false;
        if (!this.track.sps) {
          this.parseSPS(unit.data.subarray(4));
          if (!this.remuxer.readyToDecode && this.track.pps && this.track.sps) {
            this.remuxer.readyToDecode = true;
          }
        }
      ...
      case NalType.SEI:
        push = false;
        const arry = unit.data.subarray(5);
        const data = new DataView(arry.buffer, arry.byteOffset, arry.byteLength);
        let byte_idx = 0;
        //const pay_type = data.getUint8(byte_idx);
        ++byte_idx;
        //let pay_size = 0;
        ...
        //const uuid = arry.subarray(byte_idx, byte_idx + 16);
        byte_idx += 16;
        // Log.info(
    ...
    if (push === null && unit.nri > 0) {
      push = true;
    }
    return push;
  }

  public static parceSliceHeader(data: Uint8Array): number {
    const decoder = new ExpGolomb(data);
    decoder.readUEG(); // first_mb
    const slice_type = decoder.readUEG();
    decoder.readUEG(); // ppsid
    decoder.readUByte(); // framenum
    // Log.info(`first_mb: ${first_mb}, slice_type: ${slice_type}, ppsid: ${ppsid}, frame_num: ${frame_num}`);
    return slice_type;
  }
   ...
   */
  public static skipScalingList(decoder: ExpGolomb, count: number): void {
    let lastScale = 8,
      nextScale = 8,
      deltaScale: number;
  ...
  public static readSliceType(decoder: ExpGolomb): number {
    // skip NALu type
    decoder.readUByte();
